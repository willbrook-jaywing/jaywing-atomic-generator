...
var _promptModuleActionObj = {type: 'list', name: 'moduleAction', message: 'Action:', choices: ['Create New', 'Use Existing', 'None']};
...
var baseExtension = {
    ...
    return this.prompt(_promptModuleActionObj).then(function (answers) {
      ...
      switch(answers.moduleAction) {
        case 'Create New':
          ...
          this.log('  JS class: ' + _jsClassName);
          this._promptSizes();
          break;
        case 'Use Existing':
    ...
    if(exists) {
      ...
      return this.prompt(_promptNamesObj).then(function (answers) {
        ...
        for(var i = 0; i < answers.name.length; i++) {
          ...
          var namePath = this._getNamePath(answers.name[i]);
        }
        this._promptDelete();
      }.bind(this));
    } else {
  ...
  _promptDelete: function () {
    return this.prompt(_promptDeleteObj).then(function (answers) {
      answers.delete == 'Yes' ? this._delete() : this.composeWith('jaywing-atomic:app');
    }.bind(this));
  },
  ...
  _add: function () {
    ...
    for (var p = 0; p < docsTemplateDataFileObj.docs_pages.length; p++) {
      var docsPageObj = docsTemplateDataFileObj.docs_pages[p];
      if (docsPageObj.category == _category && docsPageObj.section == 'components') {
        docsPageObjFound = true;
        docsTemplateDataFileObj.docs_pages[p].items.push(docsPageItemDataObj);
        break;
      }
    }
    ...
    if(!docsPageObjFound) {
      ...
      docsTemplateDataFileObj.docs_pages.push(docsPageObj);
    }
    ...
    for(var i = 0; i < fileExtensions.length; i++) {
      ...
      if(extension !== '.scss') {
        ...
        if(extension == '.size.scss') {
          for (var a = 0; a < _sizeNames.length; a++) {
  ...
  _delete: function () {
    for(var i = 0; i < _names.length; i++) {
      ...
      // Delete docs file
      filePath = _pathTemplate + this._getNamePath(name) + '.njk';
